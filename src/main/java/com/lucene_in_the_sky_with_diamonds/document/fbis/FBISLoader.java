package com.lucene_in_the_sky_with_diamonds.document.fbis;
...
public class FBISLoader {
  ...
  public void loadDocumentsFromFile(String fileName) {
    ...
    try {
      ...
      System.out.println(
          String.format("An exception occurred when parsing the documents from %s", fileName));
      e.printStackTrace();
    }
  }
  ...
  private String buildToplevelRegex() {
    String regex =
        FIBSFieldTypes.DOC_NO_START.fieldType + "|" + FIBSFieldTypes.HEADER_START.fieldType + "|"
            + FIBSFieldTypes.TEXT_START.fieldType + "|" + FIBSFieldTypes.TITLE_START.fieldType

            + "|" + FIBSFieldTypes.HT_START.fieldType + "|" + FIBSFieldTypes.DATE1_START.fieldType;
  ...
  private FBISFieldsObject createFBISFieldsObject(ArrayList<String> sections) {
     ...
     * 
     * Whatever is left after parsing is the unknownSect. Need to do research into what to do with
     */
  ...
  private Document createDocument(FBISFieldsObject sections) throws Exception {
    ...
    /*
     * Ignoring HT, dont know what on earth that is. There is a summary section might be a good idea
     * to use it (index 6 of sections)
     * 
     * 
     */
    if (sections == null) {
